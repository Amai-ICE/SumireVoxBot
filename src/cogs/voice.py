...
class Voice(commands.Cog):
    ...
    async def config(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(name="メンション読み上げ", value="✅ 有効" if settings.read_mention else "❌ 無効", inline=True)
            embed.add_field(name="添付ファイル通知", value="✅ 有効" if settings.read_attachments else "❌ 無効", inline=True)
            embed.add_field(name="入退出通知", value="✅ 有効" if settings.read_vc_status else "❌ 無効", inline=True)

            embed.add_field(name="コードブロック省略", value="✅ 有効" if settings.skip_code_blocks else "❌ 無効", inline=True)
            embed.add_field(name="URL省略", value="✅ 有効" if settings.skip_urls else "❌ 無効", inline=True)
            # 3列のレイアウトを保つための空フィールド
            embed.add_field(name="\u200b", value="\u200b", inline=True)

            # 自動接続設定（このBot固有の設定を強調）
            bot_key = str(self.bot.user.id)
            auto_join_status = "❌ 無効"
            if settings.auto_join and bot_key in settings.auto_join_config:
                ...
                if vc and tc:
                    ...
                    auto_join_status = "⚠️ 設定不備（チャンネルが見つかりません）"
